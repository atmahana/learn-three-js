  ...
  loader.load(
    ...
    (model1) => {
      ...
      loader.load(
        "/porsche.glb",
        (model2) => {
          loadingContainer.style.display = "none"; //hide loading bar after loaded all models
          car = model2.scene;
        ...
        (xhr) => {
          bar.set((xhr.loaded / xhr.total) * 100); //update loading bar on progress
        },
        (err) => {
          console.error(err);
        }
...
sceneFolder
  .add(sceneControl, "density", 0, 0.15)
  .name("Fog Density")
  .onChange((e) => {
    scene.fog.density = e;
  });
sceneFolder
  .addColor(sceneControl, "color")
  .name("Fog Color")
  .onChange((e) => {
    scene.fog.color.set(e);
  });
sceneFolder
  .addColor(sceneControl, "signLightColor")
  .name("Sign Light Color")
  .onChange((e) => {
    signLight.color.set(e);
  });
sceneFolder
  .add(sceneControl, "signHelper")
  .name("Toggle Helper")
  .onChange((e) => {
    if (e === true) lightGroup.add(signLightHelper);
    else lightGroup.remove(signLightHelper);
  });
...
dlFolder
  .add(dlControl, "intensity", 0, 1)
  .name("Light Intensity")
  .onChange((e) => {
    dl.intensity = e;
  });
dlFolder
  .addColor(dlControl, "color")
  .name("Light Color")
  .onChange((e) => {
    dl.color.set(e);
  });
dlFolder
  .add(dlControl, "helper")
  .name("Toggle Helper")
  .onChange((e) => {
    if (e === true) lightGroup.add(dlHelper);
    else lightGroup.remove(dlHelper);
  });
...
hlFolder
  .addColor(hlControl, "groundColor")
  .name("Ground Color")
  .onChange((e) => {
    hl.groundColor.set(e);
  });
hlFolder
  .add(hlControl, "intensity", 0, 1)
  .name("Light Intensity")
  .onChange((e) => {
    hl.intensity = e;
  });
hlFolder
  .add(hlControl, "helper")
  .name("Toggle Helper")
  .onChange((e) => {
    if (e === true) lightGroup.add(hlHelper);
    else lightGroup.remove(hlHelper);
  });
